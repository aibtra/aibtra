...
class DiffManager(
	...
	private var data: Data = Data(Input(FilteredText.Part.of(""), "", ScrollPos(0, 0), "", ScrollPos.INITIAL, INITIAL_CONFIG, true, null), State(listOf(), FilteredText.asIs(FilteredText.Part.of("")), "", listOf(), Diff.INITIAL, false), 0, ScrollPos.INITIAL, ScrollPos.INITIAL)
	...
	private fun updateState(input: Input, forceUpdate: Boolean, debugOperationName: String?) {
		...
		sequentialRunner.schedule(object : Run {
			override suspend fun invoke(callback: Callback, coroutineScope: CoroutineScope) {
				...
				val state = State(rawChars, filtered, refFormatted, refChars, diff, selection)
				callback {
					Ui.assertEdt()
	...
	class State(val rawChars: List<DiffChar>, val filtered: FilteredText, val refFormatted: String, val refChars: List<DiffChar>, val diff: Diff, val selection: Boolean)
