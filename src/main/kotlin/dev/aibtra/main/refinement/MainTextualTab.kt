...
package dev.aibtra.main.refinement
...
internal abstract class MainTextualTab(initialWorkingMode: WorkingMode, private val tabbedPane: MainTabbedPane, environment: Environment, dialogDisplayer: DialogDisplayer) : MainTab(tabbedPane, environment, dialogDisplayer) {
	...
	init {
		...
		rawTextArea = RawTextArea({ text -> diffManager.updateRawText(text, null, profileManager.profile().diffConfig, normalization = Normalization.INITIALIZE) }, environment)
	...
	override fun addFileActions(fileMenu: JMenu) : Boolean {
		addAction(fileMenu, NewAction(tabbedPane, environment, this.dialogDisplayer))
		addAction(fileMenu, OpenAction(tabbedPane, environment, this.dialogDisplayer))
