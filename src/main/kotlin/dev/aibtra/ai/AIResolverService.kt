...
class AIResolverService(driver: AIDriver, apiToken: String?, private val debugLog: DebugLog, val paths: ApplicationPaths) : AIService(driver, apiToken, debugLog) {
	...
	private fun requestSingleStage(profileName: AIProfile.Name, approach: AIResolverConfiguration.SingleStageApproach, snippets: ResolverSnippets, failureHandler: FailureHandler, callback: Callback) {
		...
		val resolveOutput = sendRequest(approach.model, approach.modelParams, approach.completionsEndpoint, approach.reasoningFilter, resolveInput, failureHandler, "request") ?: return
	...
	private fun requestSummarizeMergeResolve(profileName: AIProfile.Name, approach: AIResolverConfiguration.SummarizeMergeResolveApproach, snippets: ResolverSnippets, resolverPacket: ResolverPacket?, failureHandler: FailureHandler, callback: Callback) {
		...
		val merges = (resolverPacket as? ResolverSummarizeMergeResolvePacket)?.merges ?: run {
			...
			val summarizeOutput = sendRequest(approach.model, approach.modelParams, approach.completionsEndpoint, approach.reasoningFilter, summarizeInput, failureHandler, "summarize") ?: return
			...
			val mergeOutput = sendRequest(approach.model, approach.modelParams, approach.completionsEndpoint, approach.reasoningFilter, mergeInput, failureHandler, "merge") ?: return
		...
		callback.startResolutions()
		val resolveOutput = sendRequest(approach.model, approach.modelParams, approach.completionsEndpoint, approach.reasoningFilter, resolveInput, failureHandler, "resolve") ?: return
	...
	private fun sendRequest(model: String, params: String?, completionsEndpoint: String?, reasoningFilter: String?, messages: JSONArray, failureHandler: FailureHandler, stepId: String): String? {
		...
		request(model, params, completionsEndpoint, reasoningFilter, messages, object : ResultHandler {
