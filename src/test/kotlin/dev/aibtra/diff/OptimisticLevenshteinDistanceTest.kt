...
class OptimisticLevenshteinDistanceTest {
	...
	fun testLimitNotReached1() {
		assert("kitten", "kitten", 0, maxEditDistance = 1)
	}

	@Test
	fun testLimitNotReached2() {
		assert("kitten", "kittten", 1, maxEditDistance = 1)
	}

	@Test
	fun testLimitReached() {
		assert("kitten", "sittten", -1, maxEditDistance = 1)
	}

	private fun assert(s1: String, s2: String, expected: Int, maxEditDistance : Int = Integer.MAX_VALUE) {
		val actual = OptimisticLevenshteinDistance.compute(s1, s2) { _, _, editDistance -> editDistance > maxEditDistance }.distance()
